...
<body>
    <div class="header">
        <div class="company-logo">{{ $response->campaign->company->name }}</div>
        <h1>Reporte de Evaluación Psicológica</h1>
        <div class="info">
            <strong>Participante:</strong> {{ $response->respondent_name }}<br>
                ...
                @if(isset($responses[$questionId]))
                    <div class="question-block">
                        ...
                        <div class="answer-text">
                            <strong>Respuesta:</strong> "{{ $responseText }}"
                        </div>
                    </div>
                @endif
            @endforeach
    ...
    @if($response->comprehensive_report)
        <div class="section">
            <h2>Evaluación Psicológica Profesional</h2>
            <div class="report-content">
                ...
                @if(isset($response->comprehensive_report['generated_at']))
                    <p style="margin-top: 20px; color: #666; font-size: 12px;">
                        <em>Análisis generado el {{ \Carbon\Carbon::parse($response->comprehensive_report['generated_at'])->format('d/m/Y H:i') }}</em>
                    </p>
                @endif
            </div>
        </div>
    ...
    <div class="footer">
        <p>Este reporte fue generado el {{ now()->format('d/m/Y') }} a las {{ now()->format('H:i') }}</p>
        <p><strong>{{ $response->campaign->company->name }}</strong> - Sistema de Evaluación Psicológica</p>
        <p style="font-style: italic; color: #999;">
            Documento confidencial. Para uso exclusivo de {{ $response->campaign->company->name }}
        </p>
    </div>
</body>
</html>